...
def print_job_detail(job_id: str):
    ...
    config = {
        "Job Name": detail.get("name") or "—",
        "Job ID": str(detail.get("public_id", "")),
        "Status": str(detail.get("status") or "—"),
        "Started": _fmt_timestamp_detail(started),
        "Finished": _fmt_timestamp_detail(finished),
        "Duration": _fmt_duration(started, finished),
        "CPU": str(detail.get("requested_cpu") or "—"),
        "RAM": f"{detail.get('requested_ram') or '—'} GB",
        "Disk": f"{detail.get('requested_disk_space') or '—'} GB",
        # "GPU RAM": f"{detail.get('requested_gpu_ram') or '—'} GB",
        "Credits used": str(detail.get("credits_debited") or "—"),
    }
    ...
    console.print(Panel(table, title="[title]Job Detail[/title]", expand=False, border_style="green"))
